/**
 * Service for managing shortcodes in the Friday Plugin
 * Handles registration, retrieval, and rendering of shortcodes
 */
...
export const shortcodeService = {
    ...
    async ensureShortcodesRegistered(markdown: string): Promise<void> {
        ...
        if (shortcodeNames.length === 0) {
            console.error('No shortcodes found in content');
            return;
        }
    ...
    /**
     * Render shortcodes in a markdown string
     * @param markdown The markdown string containing shortcodes
     * @returns The markdown with shortcodes rendered
     */
    render(markdown: string): string {
        return globalShortcode.render(markdown);
    }
}; 
